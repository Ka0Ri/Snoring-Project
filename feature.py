...
filter_banks = create_filter_banks(26, sampling_rate, 1024)
...
def feature_extract(data, spr):
    ...
    for i in range(0, n):
        ...
        # sig = sig - np.mean(sig)
        sig = sig * hw
        fea = stas_feature4(sig, sampling_rate)
...
#############################################################
#load data
scaling_factor = scaling(path + '/numerical/non_snoring_6.npy')
# scaling_factor = 1
print(scaling_factor)
no_snoring = np.load(path + '/numerical/xnon_snoring_6.npy')
n_snoring_sample = no_snoring.shape[0]
print(n_snoring_sample)
t1 = time.time()
no_snoring_fea_vecs = feature_extract(no_snoring, sampling_rate)
np.save(path + '/feature/xnon_snoring_6_s', np.array(no_snoring_fea_vecs))
t2 = time.time()
print("feature extraction", (t2 -t1)/(n_snoring_sample))

snoring = np.load(path + '/numerical/xsnoring_6.npy')
n_snoring_sample = snoring.shape[0]
print(n_snoring_sample)
t1 = time.time()
snoring_fea_vecs = feature_extract(snoring, sampling_rate)
np.save(path + '/feature/xsnoring_6_s', np.array(snoring_fea_vecs))
t2 = time.time()
print("feature extraction", (t2 -t1)/(n_snoring_sample))
